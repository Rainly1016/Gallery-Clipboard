	...
	if (displayLabel.hasOwnProperty('frame')) {
		...
		for (let p in frameTypeSelector) {
			if (frameTypeSelector[p].test(type)) {
				work.frame = float(p);
				console.log(type.match(frameTypeSelector[p]));
				break;
			}
		}
	}
	console.log(work.frame);
...
function CreateElements(work, infoElement, img) {
	// if (ready) {
	// 	workBlocks.attribute('display', 'inline-block');
	// }
	if (showingExample) {
...
function checkDisplayPreferences() {
	...
	for (let i = 0; i < headers.length; i++) {
		...
		checkboxs.changed(() => {
			...
			exampleBlock.remove();
			let img = createImg(IMGs[index].data, `${index}`, '', (img) => CreateObject(img));
		});
	}
	let check = createButton('确定');
	check.class('button');
	check.id('checkbutton');
	check.parent(checkBlock);
	check.mousePressed(() => {
		exampleBlock.remove();
		checkBlock.remove();
...
function handleIMG(img) {
	if (img.type === "image") {
		...
		let number = img.name.match(regexp);
		number = int(number[number.length - 1]);
		let insert = IMGs.length;
		let dist = 10000;
		if (IMGs.length > 0) {
			for (let i = 0; i < IMGs.length; i++) {
				let m = IMGs[i].name.match(regexp);
				m = int(m[m.length - 1]);
				let diff = m - number;
				if (diff > 0 && diff < dist) {
					dist = diff;
